...
import { FaRocket, FaPlus, FaLock, FaSuitcase } from 'react-icons/fa';
      ...
      >
        <div className={`flex mb-10 items-baseline ${isCollapsed ? 'flex-col gap-2' : 'flex-row'}`}>
          ...
          >
            ...
            >
              <FaSuitcase className={`${isCollapsed ? '' : 'mr-2'}`} />
            </Link>
            <span className={`${isCollapsed ? 'hidden' : 'inline font-medium'}`}>Demari RAG Demo</span>
          </div>
        </div>
